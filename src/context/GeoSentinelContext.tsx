...
// Dynamic Mine Dataset Generator for any Indian Coalfield Basin
export const buildMineDataset = (mineId: string) => {
  const targetMine = AVAILABLE_MINES.find(m => m.id === mineId) || AVAILABLE_MINES[0];
  const bLat = targetMine.lat;
  const bLng = targetMine.lng;
  const mCode = targetMine.code.split('-')[1] || 'JH';
  const shortName = targetMine.name.split(' ')[0];

  const nodes: SensorNode[] = [
    {
      id: 'NODE-A',
      name: `Node A (${shortName} S1 Ridge)`,
      code: `ESP32-S3-${mCode}-S1-A`,
      type: 'multi_sensor_node',
      hardwareModel: 'ESP32-S3 (BNO085 + BME280 + Soil Moisture)',
      zone: 'Sector 1 (North Overburden Slope)',
      sector: 1,
      lat: +(bLat + 0.0010).toFixed(6),
      lng: +(bLng - 0.0005).toFixed(6),
      elevationMeters: 235,
      meshHopCount: 1,
    ...
    {
      id: 'NODE-B',
      name: `Node B (${shortName} S1 Bench Creep)`,
      code: `ESP32-S3-${mCode}-S1-B`,
      type: 'multi_sensor_node',
      hardwareModel: 'ESP32-S3 (BNO085 + BME280 + Soil Moisture)',
      zone: 'Sector 1 (North Overburden Slope)',
      sector: 1,
      lat: +(bLat - 0.0015).toFixed(6),
      lng: +(bLng + 0.0020).toFixed(6),
      elevationMeters: 224,
      meshHopCount: 1,
      parentNodeId: 'MASTER-S1',
      masterId: 'MASTER-S1',
    ...
    {
      id: 'NODE-C',
      name: `Node C (${shortName} S1 Perimeter)`,
      code: `ESP32-S3-${mCode}-S1-C`,
      type: 'multi_sensor_node',
      hardwareModel: 'ESP32-S3 (BNO085 + BME280 + Soil Moisture)',
      zone: 'Sector 1 (North Overburden Slope)',
      sector: 1,
      lat: +(bLat + 0.0035).toFixed(6),
      lng: +(bLng - 0.0040).toFixed(6),
      elevationMeters: 240,
      meshHopCount: 2,
      parentNodeId: 'NODE-A',
      masterId: 'MASTER-S1',
    ...
    {
      id: 'NODE-X',
      name: `Node X (${shortName} S2 Village Buffer)`,
      code: `ESP32-S3-${mCode}-S2-X`,
      type: 'multi_sensor_node',
      hardwareModel: 'ESP32-S3 (BNO085 + BME280 + Soil Moisture)',
      zone: 'Sector 2 (East Highwall & Village Buffer)',
      sector: 2,
      lat: +(bLat - 0.0038).toFixed(6),
      lng: +(bLng - 0.0025).toFixed(6),
      elevationMeters: 228,
      meshHopCount: 1,
    ...
    {
      id: 'NODE-Y',
      name: `Node Y (${shortName} S2 Highwall Gallery)`,
      code: `ESP32-S3-${mCode}-S2-Y`,
      type: 'multi_sensor_node',
      hardwareModel: 'ESP32-S3 (BNO085 + BME280 + Soil Moisture)',
      zone: 'Sector 2 (East Highwall & Village Buffer)',
      sector: 2,
      lat: +(bLat - 0.0052).toFixed(6),
      lng: +(bLng + 0.0010).toFixed(6),
      elevationMeters: 218,
      meshHopCount: 1,
      parentNodeId: 'MASTER-S2',
      masterId: 'MASTER-S2',
    ...
    {
      id: 'NODE-Z',
      name: `Node Z (${shortName} S2 Incline Portal)`,
      code: `ESP32-S3-${mCode}-S2-Z`,
      type: 'multi_sensor_node',
      hardwareModel: 'ESP32-S3 (BNO085 + BME280 + Soil Moisture)',
      zone: 'Sector 2 (East Highwall & Village Buffer)',
      sector: 2,
      lat: +(bLat - 0.0068).toFixed(6),
      lng: +(bLng - 0.0055).toFixed(6),
      elevationMeters: 212,
      meshHopCount: 2,
      parentNodeId: 'NODE-Y',
      masterId: 'MASTER-S2',
    ...
    {
      id: 'NODE-D',
      name: `Node D (${shortName} S3 Tailings Dam)`,
      code: `ESP32-S3-${mCode}-S3-D`,
      type: 'multi_sensor_node',
      hardwareModel: 'ESP32-S3 (BNO085 + BME280 + Soil Moisture)',
      zone: 'Sector 3 (South Tailings Embankment)',
      sector: 3,
      lat: +(bLat - 0.0055).toFixed(6),
      lng: +(bLng - 0.0045).toFixed(6),
      elevationMeters: 205,
      meshHopCount: 1,
      parentNodeId: 'MASTER-S2',
      masterId: 'MASTER-S2',
    ...
    {
      id: 'NODE-E',
      name: `Node E (${shortName} S4 Haulage)`,
      code: `ESP32-S3-${mCode}-S4-E`,
      type: 'multi_sensor_node',
      hardwareModel: 'ESP32-S3 (BNO085 + BME280 + Soil Moisture)',
      zone: 'Sector 4 (West Haulage Bench)',
      sector: 4,
      lat: +(bLat - 0.0080).toFixed(6),
      lng: +(bLng + 0.0035).toFixed(6),
      elevationMeters: 210,
      meshHopCount: 1,
      parentNodeId: 'MASTER-S1',
      masterId: 'MASTER-S1',
  ...
  const gateway: GatewayDevice = {
    ...
    batteryPct: 99,
    wifiHotspotSsid: `GEOSENTINEL_${mCode}_MASTER`,
  ...
  const assemblyPoints: EvacuationAssemblyPoint[] = [
    {
      id: `AP-${mCode}-01`,
      name: `${shortName} High Ridge Community Hall`,
      zone: 'Sector 4 High Ground',
      lat: +(bLat - 0.0090).toFixed(6),
      lng: +(bLng + 0.0030).toFixed(6),
      elevationMeters: 262,
      capacityPersons: 450,
      currentCheckedIn: 84,
      status: 'Open & Safe',
    ...
    {
      id: `AP-${mCode}-02`,
      name: `${shortName} Central School Stadium`,
      zone: 'Sector 1 Safe Buffer',
      lat: +(bLat + 0.0070).toFixed(6),
      lng: +(bLng - 0.0050).toFixed(6),
      elevationMeters: 255,
      capacityPersons: 800,
  ...
  const reports: CitizenCrackReport[] = [
    {
      id: `CR-${Math.floor(801 + Math.random() * 50)}`,
      ...
      description: `Ground crack running across courtyard near primary school in ${shortName}, expanded overnight.`,
      severity: 'Moderate Shear Fissure',
      status: 'Corroborated & Approved',
      reviewedBy: 'Chief Geologist R. V. Sharma',
      reviewNotes: 'Matches extensometer displacement trend (+1.8mm/hr). Corroborated.',
      nearestSensorId: 'NODE-X',
      nearestSensorDistanceM: 145.0,
      aiCorroborationConfidence: 94.2
    },
    {
      id: `CR-${Math.floor(851 + Math.random() * 50)}`,
      reporterName: 'Sujata Devi',
      phone: '+91 98210 44299',
      timestamp: '2 hours ago',
      timeEpoch: Date.now() - 120 * 60 * 1000,
      lat: +(bLat - 0.0058).toFixed(6),
      lng: +(bLng - 0.0052).toFixed(6),
      zone: 'Sector 3 (Abandoned Gallery)',
      crackWidthEstimateMm: 16,
      photoUrl: SVG_CRACK_WALL,
      description: `Wall splitting along eastern foundation of pump house in ${shortName}.`,
      severity: 'Severe Subsidence Crack',
      status: 'Corroborated & Approved',
      reviewedBy: 'Mine Safety Inspector T. Sen',
      reviewNotes: 'Close to void gallery; high shear zone verified.',
  ...
  const links: MeshLink[] = [
    ...
    { id: 'L7', sourceId: 'NODE-D', targetId: 'MASTER-S2', rssiDbm: -70, packetLossPct: 0.2, protocol: 'WiFi Mesh', linkType: 'mesh_leaf', active: true },
    { id: 'L8', sourceId: 'NODE-E', targetId: 'MASTER-S1', rssiDbm: -76, packetLossPct: 0.3, protocol: 'WiFi Mesh', linkType: 'mesh_leaf', active: true },
  ...
  return { nodes, gateway, assemblyPoints, reports, links };
};

const initialMineDataset = buildMineDataset('jharia-04');
...
export const GeoSentinelProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const [selectedMine, setSelectedMineState] = useState<string>('jharia-04');
  const [selectedSector, setSelectedSectorState] = useState<string>('all');

  const [nodes, setNodes] = useState<SensorNode[]>(initialMineDataset.nodes);
  const [gateway, setGateway] = useState<GatewayDevice>(initialMineDataset.gateway);
  const [links, setLinks] = useState<MeshLink[]>(initialMineDataset.links);
  ...
  const [reports, setReports] = useState<CitizenCrackReport[]>(initialMineDataset.reports);
  const [assemblyPoints, setAssemblyPoints] = useState<EvacuationAssemblyPoint[]>(initialMineDataset.assemblyPoints);
  ...
  // Dynamic Mine Selection Handler (Switches entire basin telemetry & geo-cluster)
  const setSelectedMine = useCallback((mineId: string) => {
    setSelectedMineState(mineId);
    const ds = buildMineDataset(mineId);
    setNodes(ds.nodes);
    setGateway(ds.gateway);
    setAssemblyPoints(ds.assemblyPoints);
    setReports(ds.reports);
    setLinks(ds.links);
    setSelectedSectorState('all');
  }, []);

  // Sector Selection Handler
  const setSelectedSector = useCallback((sectorId: string) => {
    setSelectedSectorState(sectorId);
  }, []);
